...
export async function findOrCreateUser(
  ...
  if (existingProvider) {
    ...
    const [updatedUser] = await db
      ...
      .set({
        ...
        emailVerified:
          authResult.emailVerified || existingProvider.users.emailVerified,
        updatedAt: new Date(),
      })
  ...
  if (existingUser) {
    ...
    const [hasProvider] = await db
      ...
      .where(
        and(
          eq(authProviders.userId, existingUser.id),
          eq(authProviders.provider, provider),
        ),
      )
      .limit(1);
    ...
    if (hasProvider) {
      throw new Error(`Account already has ${provider} authentication linked`);
    }

    // Validate email consistency for OAuth providers
    if (provider !== "email" && existingUser.email !== authResult.email) {
      throw new Error(
        `Email mismatch. Please login with ${existingUser.email}`,
      );
    }

    // Safe to link provider now
