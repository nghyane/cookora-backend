...
import { detectIngredients } from "../src/modules/detection/detection.service";
...
describe("detectIngredients Function", () => {
  beforeAll(() => {
    ...
    if (!process.env.OPENAI_API_KEY) {
      console.warn(
        "⚠️  OPENAI_API_KEY not set - detection tests will be skipped",
      );
    }
  });
    ...
    try {
      ...
      if (result.detectedIngredients.length > 0) {
        ...
        result.detectedIngredients.forEach((ing, idx) => {
          console.log(
            `  ${idx + 1}. ${ing.name} (confidence: ${ing.confidence.toFixed(2)})`,
          );
        });
      }
  ...
  test("should work with Gemini provider if configured", async () => {
    ...
    try {
      ...
      if (result.detectedIngredients.length > 0) {
        console.log("🤖 Gemini detected ingredients:");
        result.detectedIngredients.forEach((ing, idx) => {
          console.log(
            `  ${idx + 1}. ${ing.name} (confidence: ${ing.confidence.toFixed(2)})`,
          );
